# 4.XStream类混叠 - XStream教程
类混叠是用来创建一个类的XML完全限定名称的别名。让我们修改原来的例子，下面的代码添加到它。

```
xstream.alias("student",  Student.class); xstream.alias("note",  Note.class);
```
...
```
<?xml version="1.0" encoding="UTF-8"?>  <student>  <studentName>Mahesh</studentName>  <notes>  <note>  <title>first</title>  <description>My first assignment.</description>  </note>  <note>  <title>second</title>  <description>My Second assignment.</description>  </note>  </notes>  </student>
```